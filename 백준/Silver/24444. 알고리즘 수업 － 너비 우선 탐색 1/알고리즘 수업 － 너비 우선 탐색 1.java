...
public class Main {
    ...
    public static void bfs(int startNode) {
        Queue<Integer> queue = new LinkedList<>();  // 큐를 사용하여 BFS 탐색을 진행합니다.
        visited[startNode] = order++;  // 시작 노드를 방문 순서대로 표시하고, 순서를 증가시킵니다.
        queue.offer(startNode);  // 시작 노드를 큐에 추가합니다.

        while (!queue.isEmpty()) {
            int node = queue.poll();  // 큐에서 노드를 하나 꺼냅니다.
            // 인접한 정점들을 오름차순으로 탐색합니다.
            for (int next : adjList[node]) {
                if (visited[next] == 0) {  // 아직 방문하지 않았다면
                    visited[next] = order++;  // 정점을 방문 순서대로 표시합니다.
                    queue.offer(next);  // 큐에 정점을 추가합니다.
                }
            }
        }
    }
